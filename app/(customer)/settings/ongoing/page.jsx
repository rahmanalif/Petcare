...
export default function OngoingDetails() {
  ...
  useEffect(() => {
    if (successMessage) {
        alert("Reschedule request sent successfully!"); // ইউজারকে ফিডব্যাক দেওয়া
        dispatch(clearBookingMessages());
        setRescheduleForm({ reason: "", startDate: "", endDate: "" }); // ফর্ম রিসেট
        setActiveView("details"); // ডিটেইলস পেজে ফেরত নেওয়া
    }
  ...
  // Available Days Logic (Mock logic based on calendarEvents or simply assuming future dates are hoverable)
  // এখানে লজিক: যদি ইভেন্ট স্ট্যাটাস 'available' হয় অথবা ক্যালেন্ডারে কিছু না থাকে তবে আমরা গ্রিন দেখাবো (আপনার API ডাটার ওপর নির্ভর করে)
  const availableDays = calendarEvents
    .filter(event => new Date(event.date).getMonth() === currentDate.getMonth() && event.status === 'available')
  ...
  // 🔥 Reschedule API Integration Handler
            ...
            <div className="w-full lg:w-1/3 bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                ...
                <div className="grid grid-cols-7 gap-y-4 text-center">
                    ...
                    {calendarDays.map((d, i) => {
                        ...
                        // Check if available (based on API or logic)
                        ...
                        return (
                            <div key={i} className={`flex justify-center`}>
                                <div className={`
                                    ...
                                    /* 🔥 Available Hover Logic (Green on Hover) */
                                    ${d.isCurrentMonth && !isBooked 
                                        ? "hover:bg-[#10B981] hover:text-white cursor-pointer hover:shadow-md" 
                                        : ""}
                                    
                                    /* If explicitly available in API data */
                                    ${d.isCurrentMonth && availableDays.includes(d.day) && !isBooked 
                                        ? "bg-[#E6FFFA] text-[#10B981] font-medium" 
                                        : ""}
                                `}>
                                    {d.day}
                                </div>
            ...
            <div className="w-full lg:w-2/3 space-y-4">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm relative">
                    ...
                    <div className="flex gap-6 text-sm text-[#024B5E] mb-4">
                        ...
                        <span className="flex items-center gap-1 text-gray-500"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> Live location</span>
                    </div>
                    <div className="space-y-2 mb-6">
                        <p className="text-sm font-semibold text-[#024B5E]">Contact</p>
                        <div className="flex items-center gap-2 text-xs text-gray-500"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg> {booking.owner?.phoneNumber}</div>
                    ...
                    <div className="border-t border-gray-100 pt-4 space-y-2">
                        ...
                        <div className="flex justify-between text-xs text-[#024B5E]"><span>Base Price</span><span>${booking.priceBreakdown?.basePrice || 0}</span></div>
                        {booking.priceBreakdown && Object.entries(booking.priceBreakdown).map(([key, value]) => {
                             if (key !== 'basePrice' && key !== 'platformFee' && value > 0) return (<div key={key} className="flex justify-between text-xs text-[#024B5E]"><span className="capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</span><span>${value}</span></div>);
                             return null;
                        })}
                        <div className="flex justify-between text-sm font-bold text-[#024B5E] pt-2 border-t border-gray-100 mt-2"><span>Total</span><span>${booking.totalPrice}</span></div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    <div onClick={() => setShowPetDetails(!showPetDetails)} className="flex items-center justify-between p-6 cursor-pointer hover:bg-gray-50 transition-colors">
                        <div className="flex items-center gap-3">
                            ...
                            <div><h4 className="font-bold text-[#024B5E] text-lg">{pet.name}</h4><p className="text-sm text-gray-500">{pet.breed}</p></div>
                        </div>
                    ...
                    {showPetDetails && (
                        <div className="p-6 pt-0 space-y-6">
                            ...
                            <div className="p-4 border border-gray-100 rounded-lg">
                                ...
                                <div className="mt-2"><span className="text-[#024B5E] font-bold text-sm">Pill</span><p className="text-gray-500 text-sm mt-1">{pet.careInfo?.pill || "N/A"}</p></div>
                            </div>
                            <div className="p-4 border border-gray-100 rounded-lg">
                                <SectionHeading title="Care info" icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="2"/><path d="M6 12h.01M18 12h.01"/></svg>} />
                                <div className="mb-2"><span className="text-[#024B5E] font-bold text-sm block">Veterinary info</span><div className="text-gray-500 text-sm mt-1 space-y-1"><p>Vet's Name: {pet.vetInfo?.name}</p><p>Clinic: {pet.vetInfo?.clinicName}</p><p>Address: {pet.vetInfo?.address}</p><p>Number: {pet.vetInfo?.contactNumber}</p></div></div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                    <button onClick={() => setActiveView("reschedule")} className="bg-[#FE6C5D] hover:bg-[#E65B4C] text-white font-medium px-6 py-2.5 rounded-lg shadow-sm text-sm transition-colors">Reschedule</button>
                    <button onClick={() => setActiveView("chat")} className="bg-[#035F75] hover:bg-[#024B5E] text-white font-medium px-8 py-2.5 rounded-lg shadow-sm text-sm transition-colors flex items-center gap-2"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg> Chat</button>
                </div>
            </div>
          </div>
           ...
           <div className="max-w-md mx-auto bg-white p-8 rounded-xl shadow-lg mt-10 border border-gray-200">
              ...
              <div className="flex gap-3">
                 ...
                 <button 
                    ...
                    className="flex-1 bg-[#035F75] text-white font-medium py-2.5 rounded-lg hover:bg-[#024B5E] transition-colors disabled:opacity-70 flex justify-center items-center"
                 >
