...
export default function BookingModalDaycare({ isOpen, onClose, providerData }) {
  ...
  const daycareService = useMemo(
    () =>
      (Array.isArray(profileServices) ? profileServices : []).find(
        (item) => String(item?.serviceType || "").toLowerCase() === "daycare"
      ),
    [profileServices]
  );
  ...
  const extraServiceOptions = useMemo(() => {
    const rates =
      daycareService?.rates && typeof daycareService.rates === "object"
        ? daycareService.rates
        : {};
  ...
  const basePrice = useMemo(() => {
    return Number(daycareService?.rates?.base ?? 0);
  }, [daycareService]);
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <button
                ...
                className={`border rounded-lg p-2 sm:p-4 text-[10px] sm:text-xs text-left font-montserrat ${
