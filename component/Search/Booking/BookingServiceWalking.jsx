...
export default function BookingModalWalking({ isOpen, onClose, providerData }) {
  ...
  const walkingService = useMemo(
    () =>
      (Array.isArray(profileServices) ? profileServices : []).find(
        (item) => String(item?.serviceType || "").toLowerCase() === "walking"
      ),
    [profileServices]
  );
  ...
  const extraServiceOptions = useMemo(() => {
    const rates =
      walkingService?.rates && typeof walkingService.rates === "object"
        ? walkingService.rates
        : {};
  ...
  const basePrice = useMemo(() => {
    return Number(walkingService?.rates?.base ?? 0);
  }, [walkingService]);
