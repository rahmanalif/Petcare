  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 md:p-8">
      <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-4 sm:mb-6">
        {t("settings.switch_profile.title")}
      </h2>

      {/* Toggle Switch */}
      <div className="flex items-center justify-between mb-6 sm:mb-8">
        <span className="text-sm sm:text-base font-medium text-[#024B5E]">
          {t("settings.switch_profile.switch_sitter")}
        </span>
        <button
          onClick={() => setIsProvider(!isProvider)}
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${isProvider ? "bg-[#024B5E]" : "bg-gray-200"
            }`}
        >
          <span
